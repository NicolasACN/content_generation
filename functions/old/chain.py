...
from .utils import load_prompts, make_variable_sentence
...
# simple_harmonizer_prompt_path = os.path.join(PROMPT_FOLDER_PATH, "SimpleHarmonizer")

# ## Prompts
# bm_review_and_feedback_system_message_prompt, bm_review_and_feedback_human_message_prompt  = load_prompts(bm_review_and_feedback_prompt_path)
# cw_review_and_feedback_system_message_prompt, cw_review_and_feedback_human_message_prompt = load_prompts(cw_review_and_feedback_prompt_path)
# tov_review_and_feedback_system_message_prompt, tov_review_and_feedback_human_message_prompt = load_prompts(tov_review_and_feedback_prompt_path)
# writing_system_message_prompt, writing_human_message_prompt = load_prompts(writing_prompt_path)
# rewriting_system_message_prompt, rewriting_human_message_prompt = load_prompts(rewriting_prompt_path)
# validation_system_message_prompt, validation_human_message_prompt = load_prompts(validation_prompt_path)
# seo_keywords_system_message_prompt, seo_keywords_human_message_prompt = load_prompts(seo_keywords_prompt_path)
# seo_writer_system_message_prompt, seo_writer_human_message_prompt = load_prompts(seo_writer_prompt_path)
# lexicon_post_processing_system_message_prompt_template, lexicon_post_processing_human_message_prompt_template = load_prompts(lexicon_post_processing_prompt_path)
# simple_harmonizer_system_message_prompt_template, simple_harmonizer_human_message_prompt_template = load_prompts(simple_harmonizer_prompt_path)
...
def make_bloc_generation_chain(bloc_cw_guidelines_formating_chain, bloc_data_prompt_formating_chain, bloc_guidelines_prompt_formating_chain, bloc_context_prompt_formating_chain, writing_chain, BM_review_feedback_chain, CW_review_feedback_chain, TOV_review_feedback_chain, validation_chain):
    bloc_generation_chain = (
        RunnablePassthrough.assign(
            cw_guidelines=bloc_cw_guidelines_formating_chain,
        ...
        | RunnablePassthrough.assign(generated_bloc=writing_chain)
        | RunnablePassthrough.assign(
            bm_review=BM_review_feedback_chain, 
            cw_review=CW_review_feedback_chain,
            tov_review=TOV_review_feedback_chain)
        | RunnablePassthrough.assign(validation=validation_chain)
    )
    return bloc_generation_chain

def make_bloc_regeneration_chain(rewriting_chain, BM_review_feedback_chain, CW_review_feedback_chain, TOV_review_feedback_chain, validation_chain):
    bloc_regeneration_chain = (
        RunnablePassthrough.assign(generated_bloc=rewriting_chain)
        | RunnablePassthrough.assign(
            bm_review=BM_review_feedback_chain,
            cw_review=CW_review_feedback_chain,
            tov_review=TOV_review_feedback_chain)
        #| RunnablePassthrough.assign(cw_review=CW_review_feedback_chain)
        | RunnablePassthrough.assign(validation=validation_chain)
    )
    return bloc_regeneration_chain

def make_bloc_cw_guidelines_prompt_formating_chain():
    def format_cw_guidelines(cw_guidelines):
        # page = page_section[0]
    ...
    def format_guidelines_prompt(bloc_guidelines: dict):
        ...
        for field_name, guidelines in bloc_guidelines.items():
            ...
            section = (
                ...
                f"Here is some reference {field_name} that perfectly applies the guidelines, the copywriting style and tone of voice, take inspiration from it: \n{reference_content}\n"
            )
...
def make_writing_chain(model, bloc_structure_model, prompt_folder):
...
def make_rewriting_chain(model, bloc_structure_model):
    ...
    prompt = ChatPromptTemplate(
        ...
        input_variables=["bm_review", "cw_review", "tov_review", "generated_bloc", "formated_hotel_data", "formated_bloc_guidelines", "cw_guidelines", "bm_guidelines"],
        partial_variables={"format_instructions": parser.get_format_instructions()},
    )
    rewriting_chain = prompt | model | parser
    return rewriting_chain

def make_BM_review_feedback_chain(model):
    ...
    input_variables=["formated_bloc_guidelines", "generated_bloc", "bm_guidelines"],
    partial_variables={"format_instructions": parser.get_format_instructions()},
    )
    runnable_bm = prompt | model | parser
    return runnable_bm

def make_CW_review_feedback_chain(model):
    ...
    input_variables=["formated_bloc_guidelines", "generated_bloc", "cw_guidelines"],
    partial_variables={"format_instructions": parser.get_format_instructions()},
    )
    runnable_cw = prompt | model | parser
    return runnable_cw

def make_TOV_review_feedback_chain(model, reference_content):
    ...
    input_variables=["generated_bloc", "reference_content"],
    partial_variables={"format_instructions": parser.get_format_instructions()},
    )
    runnable_tov = RunnablePassthrough.assign(reference_content=lambda x: reference_content) | prompt | model | parser 
    
    return runnable_tov

def make_validation_chain(model):
    ...
    prompt = ChatPromptTemplate(
        ...
        input_variables=["generated_bloc", "cw_review", "bm_review", "tov_review"],
        partial_variables={"format_instructions": parser.get_format_instructions()},
        )
...
def make_seo_keywords_generation_chain(model, cw_guidelines, keywords_guidelines):
    ...
    return (
        RunnablePassthrough.assign(
            formated_page_content= itemgetter("page_content") | RunnableLambda(lambda x: str(x)), 
            cw_guidelines = lambda x: cw_guidelines,
            keywords_guidelines = lambda x: keywords_guidelines, 
        ) | RunnablePassthrough.assign(keyword_list=prompt | model | output_parser | itemgetter('keyword_list'))
    ) 
    
def make_seo_section_generation_chain(model, cw_guidelines, keywords_guidelines):
    ...
    seo_keywords_generation_chain = make_seo_keywords_generation_chain(model, cw_guidelines, keywords_guidelines)
...
def make_lexicon_post_processing_chain(model, cw_guidelines):
    ...
    lexicon_post_processing_chain = RunnablePassthrough.assign(cw_guidelines=lambda x: cw_guidelines) | prompt | model | output_parser | itemgetter("corrected_text")
    
    return lexicon_post_processing_chain
class HarmonizedText(BaseModel):
    harmonized_text: str = Field(description="Harmonized markdown text")
    
def make_simple_harmonizer_chain(model):
    output_parser = JsonOutputParser(pydantic_object=HarmonizedText)
    prompt = ChatPromptTemplate(
        messages=[
            SystemMessagePromptTemplate.from_template(simple_harmonizer_system_message_prompt_template),
            HumanMessagePromptTemplate.from_template(simple_harmonizer_human_message_prompt_template)
        ],
        input_variables=["input_text", "cw_guidelines"],
        partial_variables={"format_instructions": output_parser.get_format_instructions()}
    )
    simple_harmonizer = prompt | model | output_parser | itemgetter("harmonized_text")
    return simple_harmonizer
